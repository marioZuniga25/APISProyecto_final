...
namespace ProyectoFinalAPI.Controllers
{
    ...
    public class ProductoController : ControllerBase
    {
        ...
        {
            _logger.LogInformation("Iniciando la validación de stock para el producto con ID: {IdProducto}", id);
            ...
            try
            {
                ...
                _logger.LogInformation("Producto con ID: {IdProducto} encontrado. Stock actual: {Stock}", id, producto.stock);

                if (producto.stock > 0)
                {
                    _logger.LogInformation("Producto con ID: {IdProducto} tiene stock disponible", id);
                    return Ok(new { mensaje = "1" });
                }
                else
                {
                    _logger.LogInformation("Producto con ID: {IdProducto} no tiene stock disponible", id);
                    return Ok(new { mensaje = "0" });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al validar el stock para el producto con ID: {IdProducto}", id);
                return StatusCode(500, "Ocurrió un error al validar el stock del producto.");
            }
        }
